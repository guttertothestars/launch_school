...
def letter_percentages(string)
  ...
  count = 0
  loop do
    if !(ALPHABET.include?(string_array[count].upcase))
      letters_hash[:neither] += 1
    elsif is_downcase?(string_array[count])
      letters_hash[:lowercase] += 1
    elsif is_upcase?(string_array[count])
      letters_hash[:uppercase] += 1
    end
    count += 1
  ...
  letters_hash.each do |key, value|
    percentages[key] = (letters_hash[key].to_f / string_array.size.to_f * 100).round(1)
  end
  percentages
end
